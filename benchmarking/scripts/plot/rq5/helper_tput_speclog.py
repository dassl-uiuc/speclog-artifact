...
with open("{0}-tput".format(mode), 'w') as f:
    for j in range(0, len(timestamps)):
        curr_ts = to_relative_ms(timestamps[j])
        if curr_ts >= zs and curr_ts <= ze: 
            f.write(str(curr_ts - zs) + "\t" + str(tput_values[j]) + "\t" + str(to_relative_ms(total_tput_timestamps[j]) - zs) + "\t" + str(total_tput_values[j]) + "\n")

burst_start_rel = to_relative_ms(burst_start_time)
print(burst_start_rel-zs)
'''
# Plot
...
plt.tight_layout()
plt.savefig("{0}_tput.png".format(mode), dpi=600)
'''