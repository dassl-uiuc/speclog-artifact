...
func appendThread(scalog *scalog_api.Scalog, id int, shardId int, timeSecs int) {
	...
	// if we are supposed to connect to shard 1, wait for view updation before creating append clients that connect to it
	if shardId/2 == 1 {
		scalog.WaitForLiveShardSize(2)
	}
...
func computationThread(c *scalog_api.Scalog) {
	...
	for {
		...
		select {
		case <-printTicker.C:
			log.Printf("[reconfig_e2e]: computed %v records", totalBatchSize)
			log.Printf("[reconfig_e2e]: average batch size %v", totalBatchSize/numBatches)
		case <-stop:
			log.Printf("[reconfig_e2e]: consumer thread terminating")
			c.Stop <- true
			return
		default:
			continue
...
func main() {
	...
	if err != nil {
		log.Errorf("[reconfig_e2e]: unable to parse with consumer")
	}

	log.Printf("[reconfig_e2e]: starting benchmark with runtime %v", runTimeSecs)
	...
	log.Printf("[reconfig_e2e]: benchmark complete")
}
