...
func main() {
	...
	if err != nil {
		log.Errorf("[lagfix]: unable to parse number of appenders per shard")
	}

	computationTime, err := strconv.Atoi(os.Args[3])
	if err != nil {
		log.Errorf("[lagfix]: unable to parse time duration")
	}
	...
	if err != nil {
		log.Errorf("[lagfix]: failed to open csv file: %v", err)
	}
	defer appendMetrics.Close()

	appendLatencyTimestamp, err := os.Create(filepath + "append_latency_timestamp.csv")
	if err != nil {
	...
	header = []string{"gsn", "delivery latency (us)", "compute latency (us)", "queuing delay (us)", "delivery timestamp"}
